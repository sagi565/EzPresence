...
const VisionPage: React.FC = () => {
  ...
  const handleUpdateSend = async (_e: React.MouseEvent<HTMLButtonElement>) => {
    if (!updatePrompt.trim() || isBusy || isUpdating) return;
  ...
  const renderPromptBox = (minimalist = false) => minimalist ? (
    <PromptBox
      ...
      isReady={updatePrompt.trim().length > 0} isBusy={isBusy || isUpdating} isLoading={isLoading} isPolling={isPolling}
      ...
      >
        ...
        {showIdle && historyDetailPlan && (
          <ReadyViewV2
            ...
            apiError={apiError}
            readonly={historyDetailStatus === 'done' || historyDetailStatus === 'in_process'}
            promptBoxSlot={!isUpdating && historyDetailStatus === 'draft' ? renderPromptBox(true) : undefined}
        ...
        {showReady && planToShow && (
          <ReadyViewV2
            ...
            apiError={apiError}
            promptBoxSlot={isUpdating ? undefined : renderPromptBox(true)}
